...
import { getByUser } from '@/api/park/park'
import { fetchList as getCarType } from '@/api/car/type'
...
const user = {
  ...
  actions: {
    ...
    GetUserInfo({commit,dispatch}) {
      return new Promise((resolve, reject) => {
        getUserInfo().then((res) => {
          ...
          if(data.sysUser && data.sysUser.userId){
            dispatch('GetParkByUser',data.sysUser.userId);
          }
    ...
    GetParkByUser({commit,dispatch},userId){
      return new Promise((resolve, reject)=>{
        getByUser(userId).then(res=>{
          const data = res.data.data || [];
          commit('SET_PARKLIST', data);
          if(data.length>0){
            const PARKINFO = getStore({ name: 'parkInfo' });
            if(PARKINFO && PARKINFO.id){
              let info = data.filter(i=>{return i.id === PARKINFO.id});
              if(info.length>0){
                commit('SET_PARKINFO', info[0]);
                dispatch('GetCarType', PARKINFO.id);
              }
              
            }else{
              commit('SET_PARKINFO', data[0]);
              dispatch('GetCarType',data[0].id);
            }
    ...
    GetCarType({commit},parkId){
      return new Promise((resolve, reject)=>{
        getCarType({parkId:parkId}).then(res=>{
          let data = res.data.data || [];
          data.push({name:'临时车',id:'L'})
